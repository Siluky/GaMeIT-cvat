...
import {
    Piece, Rotation, getBlocks, isRotation,
} from './Piece';
...
export const addPieceToBoard = (
    ...
    const filledCells = block.reduce<Array<Coords | false>>(
        (output, row, y) => output.concat(
            row.map((cell, x) => (cell ? { x: x + position.x, y: y + position.y } : false)),
        ),
        [],
    );
    ...
    return matrix.map((row, y) => row.map((cell, x) => (filled.has(serializeCoords({ x, y })) ? value : cell)));
};
...
export function setPiece(
    ...
    const updatedMatrix = addPieceToBoard(matrix, positionedPiece);
    const linesCleared = clearFullLines(updatedMatrix);
    return [updatedMatrix, linesCleared];
}
